...
class AopClient {
    ...
    public function alonersaSign($data,$privatekey,$signType = "RSA2",$keyfromfile=false) {
        ...
        ($res) or die('您使用的私钥格式错误，请检查RSA2私钥配置');
    ...
    */
    ...
    public function encryptAndSign($bizContent, $rsaPublicKeyPem, $rsaPrivateKeyPem, $charset, $isEncrypt, $isSign, $signType='RSA2') {
        ...
        if ($isEncrypt && $isSign) {
            ...
            $response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipay><response>$encrypted</response><encryption_type>RSA2</encryption_type><sign>$sign</sign><sign_type>$signType</sign_type></alipay>";
            return $response;
        }
        // 加密，不签名
        if ($isEncrypt && (!$isSign)) {
    ...
    public function rsaEncrypt($data, $rsaPublicKeyPem, $charset) {
        ...
        ($res) or die('支付宝RSA2公钥错误。请检查公钥文件格式是否正确');
